...
class CheckRecordServiceTest {
	...
	@Autowired CheckRecordService checkRecordService;
	...
	void not_change_case() {
		...
		long CHECK_MINUTES_GAP = 70L;
		long MINUTES_GAP = CHECK_MINUTES_GAP - CONNECT_MINUTES_GAP;
		...
		ConnectHistoryEntity historyEntity =
				ConnectHistoryEntity.builder()
						...
						.build();
		LocalDateTime checkTime = firstDayOfMonth.plusMinutes(CHECK_MINUTES_GAP);

		// When
		ConnectHistoryEntity response = checkRecordService.execute(historyEntity, checkTime);
		...
		Assertions.assertEquals(checkTime, response.getCheckTime());
	...
	void change_month_case() {
		...
		ConnectHistoryEntity response =
				checkRecordService.execute(historyEntity, afterMonthDisconnectTime);

		// Then
		RecordSupportInfo record = recordParser.execute(response.getRecord());
		Assertions.assertEquals(ConnectStatus.CONNECTED, response.getConnectStatus());
		Assertions.assertEquals(beforeMonthConnectTime, response.getConnectTime());
		Assertions.assertEquals(afterMonthDisconnectTime, response.getCheckTime());
	...
	void change_day_case() {
		...
		ConnectHistoryEntity historyEntity =
				ConnectHistoryEntity.builder()
						...
						.disConnectTime(connectTime)
						.build();
		LocalDateTime checkTime = secondDayOfMonth;
